...
router.get(config.github.callbackUrl, passport.authenticate('registerGithubStrategy', {failureRedirect: '/api/sessions/fail-register'}), (req, res) => {
    if (req.user) {
        res.redirect('/profile');
    } else {
        res.redirect('/api/sessions/fail-register');
    }
})
